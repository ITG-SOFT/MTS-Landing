...
const buttonClose=e.target.closest(`[${this.options.attributeCloseButton}]`);if(buttonClose||!e.target.closest(`.${this.options.classes.popupContent}`)&&this.isOpen){e.preventDefault();if(e.target.closest(".delit"))return;if(e.target.closest(".remove-all"))return;if(e.target.closest(".compare-image-block__close"))return;if(e.target.closest(".drop-search-block-list a"))return;if(e.target.closest(".drop-search-block-items a"))return;this.close();return}}.bind(this));document.addEventListener("keydown",function(e){if(this.options.closeEsc&&e.which==27&&e.code==="Escape"&&this.isOpen){e.preventDefault();this.close();return}
...
FLS(`[gotoBlock]: Юхуу...їдемо до ${targetBlock}`)}else FLS(`[gotoBlock]: Йой... Такого блоку немає на сторінці: ${targetBlock}`)};function formFieldsInit(options={viewPass:!1,autoHeight:!1}){document.body.addEventListener("focusin",(function(e){const targetElement=e.target;if(targetElement.tagName==="INPUT"||targetElement.tagName==="TEXTAREA"){if(!targetElement.hasAttribute("data-no-focus-classes")){targetElement.classList.add("_form-focus");targetElement.parentElement.classList.add("_form-focus")}
formValidate.removeError(targetElement);targetElement.hasAttribute("data-validate")?formValidate.removeError(targetElement):null}}));document.body.addEventListener("focusout",(function(e){const targetElement=e.target;if(targetElement.tagName==="INPUT"||targetElement.tagName==="TEXTAREA"){if(!targetElement.hasAttribute("data-no-focus-classes")){targetElement.classList.remove("_form-focus");targetElement.parentElement.classList.remove("_form-focus")}
...
if(flsModules.select){let selects=form.querySelectorAll("div.select");if(selects.length)for(let index=0;index<selects.length;index++){const select=selects[index].querySelector("select");flsModules.select.selectBuild(select)}}}),0)},emailTest(formRequiredItem){return!/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,8})+$/.test(formRequiredItem.value)}};function formSubmit(){const forms=document.forms;if(forms.length)for(const form of forms){form.addEventListener("submit",(function(e){const form=e.target;formSubmitAction(form,e)}));form.addEventListener("reset",(function(e){const form=e.target;formValidate.formClean(form)}))}
async function formSubmitAction(form,e){const error=!form.hasAttribute("data-no-validate")?formValidate.getErrors(form):0;if(error===0){const ajax=form.hasAttribute("data-ajax");if(ajax){e.preventDefault();const formAction=form.getAttribute("action")?form.getAttribute("action").trim():"#";const formMethod=form.getAttribute("method")?form.getAttribute("method").trim():"GET";const formData=new FormData(form);form.classList.add("_sending");const response=await fetch(formAction,{method:formMethod,body:formData});if(response.ok){let responseResult=await response.json();form.classList.remove("_sending");formSent(form,responseResult)}else{alert("Помилка");form.classList.remove("_sending")}}else if(form.hasAttribute("data-dev")){e.preventDefault();formSent(form)}}else{e.preventDefault();if(form.querySelector("._form-error")&&form.hasAttribute("data-goto-error")){const formGoToErrorClass=form.dataset.gotoError?form.dataset.gotoError:"._form-error";gotoBlock(formGoToErrorClass,!0,1e3)}}}
function formSent(form,responseResult=``){document.dispatchEvent(new CustomEvent("formSent",{detail:{form}}));setTimeout((()=>{if(flsModules.popup){const popup=form.dataset.popupMessage;popup?flsModules.popup.open(popup):null}}),0);formValidate.formClean(form);formLogging(`Форму відправлено!`)}
function formLogging(message){FLS(`[Форми]: ${message}`)}}
function formQuantity(){document.addEventListener("click",(function(e){let targetElement=e.target;if(targetElement.closest("[data-quantity-plus]")||targetElement.closest("[data-quantity-minus]")){const valueElement=targetElement.closest("[data-quantity]").querySelector("[data-quantity-value]");let value=parseInt(valueElement.value);if(targetElement.hasAttribute("data-quantity-plus")){value++;if(+valueElement.dataset.quantityMax&&+valueElement.dataset.quantityMax<value)value=valueElement.dataset.quantityMax}else{--value;if(+valueElement.dataset.quantityMin){if(+valueElement.dataset.quantityMin>value)value=valueElement.dataset.quantityMin}else if(value<1)value=1}
targetElement.closest("[data-quantity]").querySelector("[data-quantity-value]").value=value}}))}
function ssr_window_esm_isObject(obj){return obj!==null&&typeof obj==="object"&&"constructor" in obj&&obj.constructor===Object}
...
return parents}
function elementOuterSize(el,size,includeMargins){const window=ssr_window_esm_getWindow();if(includeMargins)return el[size==="width"?"offsetWidth":"offsetHeight"]+parseFloat(window.getComputedStyle(el,null).getPropertyValue(size==="width"?"margin-right":"margin-top"))+parseFloat(window.getComputedStyle(el,null).getPropertyValue(size==="width"?"margin-left":"margin-bottom"));return el.offsetWidth}
...
let browser;function calcBrowser(){const window=ssr_window_esm_getWindow();const device=getDevice();let needPerspectiveFix=!1;function isSafari(){const ua=window.navigator.userAgent.toLowerCase();return ua.indexOf("safari")>=0&&ua.indexOf("chrome")<0&&ua.indexOf("android")<0}
...
if(!swiper.animating){swiper.animating=!0;if(!swiper.onTranslateToWrapperTransitionEnd)swiper.onTranslateToWrapperTransitionEnd=function transitionEnd(e){if(!swiper||swiper.destroyed)return;if(e.target!==this)return;swiper.wrapperEl.removeEventListener("transitionend",swiper.onTranslateToWrapperTransitionEnd);swiper.onTranslateToWrapperTransitionEnd=null;delete swiper.onTranslateToWrapperTransitionEnd;swiper.animating=!1;if(runCallbacks)swiper.emit("transitionEnd");};swiper.wrapperEl.addEventListener("transitionend",swiper.onTranslateToWrapperTransitionEnd)}}
return!0}
...
var transition={setTransition,transitionStart,transitionEnd};function slideTo(index,speed,runCallbacks,internal,initial){if(index===void 0)index=0;if(runCallbacks===void 0)runCallbacks=!0;if(typeof index==="string")index=parseInt(index,10);const swiper=this;let slideIndex=index;if(slideIndex<0)slideIndex=0;const{params,snapGrid,slidesGrid,previousIndex,activeIndex,rtlTranslate:rtl,wrapperEl,enabled}=swiper;if(!enabled&&!internal&&!initial||swiper.destroyed||swiper.animating&&params.preventInteractionOnTransition)return!1;if(typeof speed==="undefined")speed=swiper.params.speed;const skip=Math.min(swiper.params.slidesPerGroupSkip,slideIndex);let snapIndex=skip+Math.floor((slideIndex-skip)/swiper.params.slidesPerGroup);if(snapIndex>=snapGrid.length)snapIndex=snapGrid.length-1;const translate=-snapGrid[snapIndex];if(params.normalizeSlideIndex)for(let i=0;i<slidesGrid.length;i+=1){const normalizedTranslate=-Math.floor(translate*100);const normalizedGrid=Math.floor(slidesGrid[i]*100);const normalizedGridNext=Math.floor(slidesGrid[i+1]*100);if(typeof slidesGrid[i+1]!=="undefined"){if(normalizedTranslate>=normalizedGrid&&normalizedTranslate<normalizedGridNext-(normalizedGridNext-normalizedGrid)/2)slideIndex=i;else if(normalizedTranslate>=normalizedGrid&&normalizedTranslate<normalizedGridNext)slideIndex=i+1}else if(normalizedTranslate>=normalizedGrid)slideIndex=i}
...
return!0}
function slideToLoop(index,speed,runCallbacks,internal){if(index===void 0)index=0;if(runCallbacks===void 0)runCallbacks=!0;if(typeof index==="string"){const indexAsNumber=parseInt(index,10);index=indexAsNumber}
...
function slideNext(speed,runCallbacks,internal){if(runCallbacks===void 0)runCallbacks=!0;const swiper=this;const{enabled,params,animating}=swiper;if(!enabled||swiper.destroyed)return swiper;if(typeof speed==="undefined")speed=swiper.params.speed;let perGroup=params.slidesPerGroup;if(params.slidesPerView==="auto"&&params.slidesPerGroup===1&&params.slidesPerGroupAuto)perGroup=Math.max(swiper.slidesPerViewDynamic("current",!0),1);const increment=swiper.activeIndex<params.slidesPerGroupSkip?1:perGroup;const isVirtual=swiper.virtual&&params.virtual.enabled;if(params.loop){if(animating&&!isVirtual&&params.loopPreventsSliding)return!1;swiper.loopFix({direction:"next"});swiper._clientLeft=swiper.wrapperEl.clientLeft;if(swiper.activeIndex===swiper.slides.length-1&&params.cssMode){requestAnimationFrame((()=>{swiper.slideTo(swiper.activeIndex+increment,speed,runCallbacks,internal)}));return!0}}
...
function slideReset(speed,runCallbacks,internal){if(runCallbacks===void 0)runCallbacks=!0;const swiper=this;if(swiper.destroyed)return;if(typeof speed==="undefined")speed=swiper.params.speed;return swiper.slideTo(swiper.activeIndex,speed,runCallbacks,internal)}
...
const prototypes={eventsEmitter,update,translate,transition,slide,loop,grabCursor,events:events$1,breakpoints,checkOverflow:checkOverflow$1,classes};const extendedDefaults={};class swiper_core_Swiper{constructor(){let el;let params;for(var _len=arguments.length,args=new Array(_len),_key=0;_key<_len;_key++)args[_key]=arguments[_key];if(args.length===1&&args[0].constructor&&Object.prototype.toString.call(args[0]).slice(8,-1)==="Object")params=args[0];else[el,params]=args;if(!params)params={};params=utils_extend({},params);if(el&&!params.el)params.el=el;const document=ssr_window_esm_getDocument();if(params.el&&typeof params.el==="string"&&document.querySelectorAll(params.el).length>1){const swipers=[];document.querySelectorAll(params.el).forEach((containerEl=>{const newParams=utils_extend({},params,{el:containerEl});swipers.push(new swiper_core_Swiper(newParams))}));return swipers}
...
slidesPerViewDynamic(view,exact){if(view===void 0)view="current";if(exact===void 0)exact=!1;const swiper=this;const{params,slides,slidesGrid,slidesSizesGrid,size:swiperSize,activeIndex}=swiper;let spv=1;if(typeof params.slidesPerView==="number")return params.slidesPerView;if(params.centeredSlides){let slideSize=slides[activeIndex]?Math.ceil(slides[activeIndex].swiperSlideSize):0;let breakLoop;for(let i=activeIndex+1;i<slides.length;i+=1)if(slides[i]&&!breakLoop){slideSize+=Math.ceil(slides[i].swiperSlideSize);spv+=1;if(slideSize>swiperSize)breakLoop=!0}
...
static installModule(mod){if(!swiper_core_Swiper.prototype.__modules__)swiper_core_Swiper.prototype.__modules__=[];const modules=swiper_core_Swiper.prototype.__modules__;if(typeof mod==="function"&&modules.indexOf(mod)<0)modules.push(mod);}
static use(module){if(Array.isArray(module)){module.forEach((m=>swiper_core_Swiper.installModule(m)));return swiper_core_Swiper}
swiper_core_Swiper.installModule(module);return swiper_core_Swiper}}
Object.keys(prototypes).forEach((prototypeGroup=>{Object.keys(prototypes[prototypeGroup]).forEach((protoMethod=>{swiper_core_Swiper.prototype[protoMethod]=prototypes[prototypeGroup][protoMethod]}))}));swiper_core_Swiper.use([Resize,Observer]);function create_element_if_not_defined_createElementIfNotDefined(swiper,originalParams,params,checkProps){if(swiper.params.createElements)Object.keys(checkProps).forEach((key=>{if(!params[key]&&params.auto===!0){let element=utils_elementChildren(swiper.el,`.${checkProps[key]}`)[0];if(!element){element=utils_createElement("div",checkProps[key]);element.className=checkProps[key];swiper.el.append(element)}
...
function onBulletClick(e){const bulletEl=e.target.closest(classes_to_selector_classesToSelector(swiper.params.pagination.bulletClass));if(!bulletEl)return;e.preventDefault();const index=utils_elementIndex(bulletEl)*swiper.params.slidesPerGroup;if(swiper.params.loop){if(swiper.realIndex===index)return;swiper.slideToLoop(index)}else swiper.slideTo(index)}
...
bullets.forEach((bulletEl=>{const classesToRemove=[...["","-next","-next-next","-prev","-prev-prev","-main"].map((suffix=>`${params.bulletActiveClass}${suffix}`))].map((s=>typeof s==="string"&&s.includes(" ")?s.split(" "):s)).flat();bulletEl.classList.remove(...classesToRemove)}));if(el.length>1)bullets.forEach((bullet=>{const bulletIndex=utils_elementIndex(bullet);if(bulletIndex===current)bullet.classList.add(...params.bulletActiveClass.split(" "));else if(swiper.isElement)bullet.setAttribute("part","bullet");if(params.dynamicBullets){if(bulletIndex>=firstIndex&&bulletIndex<=lastIndex)bullet.classList.add(...`${params.bulletActiveClass}-main`.split(" "));if(bulletIndex===firstIndex)setSideBullets(bullet,"prev");if(bulletIndex===lastIndex)setSideBullets(bullet,"next");}}));else{const bullet=bullets[current];if(bullet)bullet.classList.add(...params.bulletActiveClass.split(" "));if(swiper.isElement)bullets.forEach(((bulletEl,bulletIndex)=>{bulletEl.setAttribute("part",bulletIndex===current?"bullet-active":"bullet")}));if(params.dynamicBullets){const firstDisplayedBullet=bullets[firstIndex];const lastDisplayedBullet=bullets[lastIndex];for(let i=firstIndex;i<=lastIndex;i+=1)if(bullets[i])bullets[i].classList.add(...`${params.bulletActiveClass}-main`.split(" "));setSideBullets(firstDisplayedBullet,"prev");setSideBullets(lastDisplayedBullet,"next")}}
...
if(params.type==="progressbar"){let progressbarDirection;if(params.progressbarOpposite)progressbarDirection=swiper.isHorizontal()?"vertical":"horizontal";else progressbarDirection=swiper.isHorizontal()?"horizontal":"vertical";const scale=(current+1)/total;let scaleX=1;let scaleY=1;if(progressbarDirection==="horizontal")scaleX=scale;else scaleY=scale;subEl.querySelectorAll(classes_to_selector_classesToSelector(params.progressbarFillClass)).forEach((progressEl=>{progressEl.style.transform=`translate3d(0,0,0) scaleX(${scaleX}) scaleY(${scaleY})`;progressEl.style.transitionDuration=`${swiper.params.speed}ms`}))}
...
function render(){const params=swiper.params.pagination;if(isPaginationDisabled())return;const slidesLength=swiper.virtual&&swiper.params.virtual.enabled?swiper.virtual.slides.length:swiper.grid&&swiper.params.grid.rows>1?swiper.slides.length/Math.ceil(swiper.params.grid.rows):swiper.slides.length;let el=swiper.pagination.el;el=utils_makeElementsArray(el);let paginationHTML="";if(params.type==="bullets"){let numberOfBullets=swiper.params.loop?Math.ceil(slidesLength/swiper.params.slidesPerGroup):swiper.snapGrid.length;if(swiper.params.freeMode&&swiper.params.freeMode.enabled&&numberOfBullets>slidesLength)numberOfBullets=slidesLength;for(let i=0;i<numberOfBullets;i+=1)if(params.renderBullet)paginationHTML+=params.renderBullet.call(swiper,i,params.bulletClass);else paginationHTML+=`<${params.bulletElement} ${swiper.isElement ? 'part="bullet"' : ""} class="${params.bulletClass}"></${params.bulletElement}>`}
if(params.type==="fraction")if(params.renderFraction)paginationHTML=params.renderFraction.call(swiper,params.currentClass,params.totalClass);else paginationHTML=`<span class="${params.currentClass}"></span>`+" / "+`<span class="${params.totalClass}"></span>`;if(params.type==="progressbar")if(params.renderProgressbar)paginationHTML=params.renderProgressbar.call(swiper,params.progressbarFillClass);else paginationHTML=`<span class="${params.progressbarFillClass}"></span>`;swiper.pagination.bullets=[];el.forEach((subEl=>{if(params.type!=="custom")subEl.innerHTML=paginationHTML||"";if(params.type==="bullets")swiper.pagination.bullets.push(...subEl.querySelectorAll(classes_to_selector_classesToSelector(params.bulletClass)));}));if(params.type!=="custom")emit("paginationRender",el[0]);}
function init(){swiper.params.pagination=create_element_if_not_defined_createElementIfNotDefined(swiper,swiper.originalParams.pagination,swiper.params.pagination,{el:"swiper-pagination"});const params=swiper.params.pagination;if(!params.el)return;let el;if(typeof params.el==="string"&&swiper.isElement)el=swiper.el.querySelector(params.el);if(!el&&typeof params.el==="string")el=[...document.querySelectorAll(params.el)];if(!el)el=params.el;if(!el||el.length===0)return;if(swiper.params.uniqueNavElements&&typeof params.el==="string"&&Array.isArray(el)&&el.length>1){el=[...swiper.el.querySelectorAll(params.el)];if(el.length>1)el=el.filter((subEl=>{if(utils_elementParents(subEl,".swiper")[0]!==swiper.el)return!1;return!0}))[0]}
if(Array.isArray(el)&&el.length===1)el=el[0];Object.assign(swiper.pagination,{el});el=utils_makeElementsArray(el);el.forEach((subEl=>{if(params.type==="bullets"&&params.clickable)subEl.classList.add(...(params.clickableClass||"").split(" "));subEl.classList.add(params.modifierClass+params.type);subEl.classList.add(swiper.isHorizontal()?params.horizontalClass:params.verticalClass);if(params.type==="bullets"&&params.dynamicBullets){subEl.classList.add(`${params.modifierClass}${params.type}-dynamic`);dynamicBulletIndex=0;if(params.dynamicMainBullets<1)params.dynamicMainBullets=1}
if(params.type==="progressbar"&&params.progressbarOpposite)subEl.classList.add(params.progressbarOppositeClass);if(params.clickable)subEl.addEventListener("click",onBulletClick);if(!swiper.enabled)subEl.classList.add(params.lockClass);}))}
function destroy(){const params=swiper.params.pagination;if(isPaginationDisabled())return;let el=swiper.pagination.el;if(el){el=utils_makeElementsArray(el);el.forEach((subEl=>{subEl.classList.remove(params.hiddenClass);subEl.classList.remove(params.modifierClass+params.type);subEl.classList.remove(swiper.isHorizontal()?params.horizontalClass:params.verticalClass);if(params.clickable){subEl.classList.remove(...(params.clickableClass||"").split(" "));subEl.removeEventListener("click",onBulletClick)}}))}
if(swiper.pagination.bullets)swiper.pagination.bullets.forEach((subEl=>subEl.classList.remove(...params.bulletActiveClass.split(" "))));}
on("changeDirection",(()=>{if(!swiper.pagination||!swiper.pagination.el)return;const params=swiper.params.pagination;let{el}=swiper.pagination;el=utils_makeElementsArray(el);el.forEach((subEl=>{subEl.classList.remove(params.horizontalClass,params.verticalClass);subEl.classList.add(swiper.isHorizontal()?params.horizontalClass:params.verticalClass)}))}));on("init",(()=>{if(swiper.params.pagination.enabled===!1)disable();else{init();render();update()}}));on("activeIndexChange",(()=>{if(typeof swiper.snapIndex==="undefined")update();}));on("snapIndexChange",(()=>{update()}));on("snapGridLengthChange",(()=>{render();update()}));on("destroy",(()=>{destroy()}));on("enable disable",(()=>{let{el}=swiper.pagination;if(el){el=utils_makeElementsArray(el);el.forEach((subEl=>subEl.classList[swiper.enabled?"remove":"add"](swiper.params.pagination.lockClass)))}}));on("lock unlock",(()=>{update()}));on("click",((_s,e)=>{const targetEl=e.target;const el=utils_makeElementsArray(swiper.pagination.el);if(swiper.params.pagination.el&&swiper.params.pagination.hideOnClick&&el&&el.length>0&&!targetEl.classList.contains(swiper.params.pagination.bulletClass)){if(swiper.navigation&&(swiper.navigation.nextEl&&targetEl===swiper.navigation.nextEl||swiper.navigation.prevEl&&targetEl===swiper.navigation.prevEl))return;const isHidden=el[0].classList.contains(swiper.params.pagination.hiddenClass);if(isHidden===!0)emit("paginationShow");else emit("paginationHide");el.forEach((subEl=>subEl.classList.toggle(swiper.params.pagination.hiddenClass)))}}));const enable=()=>{swiper.el.classList.remove(swiper.params.pagination.paginationDisabledClass);let{el}=swiper.pagination;if(el){el=utils_makeElementsArray(el);el.forEach((subEl=>subEl.classList.remove(swiper.params.pagination.paginationDisabledClass)))}
init();render();update()};const disable=()=>{swiper.el.classList.add(swiper.params.pagination.paginationDisabledClass);let{el}=swiper.pagination;if(el){el=utils_makeElementsArray(el);el.forEach((subEl=>subEl.classList.add(swiper.params.pagination.paginationDisabledClass)))}
...
if(x.indexOf("%")>=0)x=`${parseInt(x, 10) * progress * rtlFactor}%`;else x=`${x * progress * rtlFactor}px`;if(y.indexOf("%")>=0)y=`${parseInt(y, 10) * progress}%`;else y=`${y * progress}px`;if(typeof opacity!=="undefined"&&opacity!==null){const currentOpacity=opacity-(opacity-1)*(1-Math.abs(progress));el.style.opacity=currentOpacity}
...
function initSliders(){if(document.querySelector(".main-slider__slider"))new swiper_core_Swiper(".main-slider__slider",{modules:[Navigation,Parallax,Pagination],observer:!0,observeParents:!0,slidesPerView:1,spaceBetween:0,speed:2e3,parallax:!0,pagination:{el:".swiper-pagination",clickable:!0},navigation:{prevEl:".swiper-button-prev",nextEl:".swiper-button-next"},on:{}});if(document.querySelector(".main-popular-block__slider"))new swiper_core_Swiper(".main-popular-block__slider",{modules:[Navigation],observer:!0,observeParents:!0,slidesPerView:5,spaceBetween:20,speed:800,parallax:!0,breakpoints:{320:{slidesPerView:2.1,spaceBetween:20},350:{slidesPerView:2.3,spaceBetween:20},490:{slidesPerView:3.5,spaceBetween:20},640:{slidesPerView:3.2,spaceBetween:20},768:{slidesPerView:3,spaceBetween:20},992:{slidesPerView:4,spaceBetween:20},1268:{slidesPerView:5,spaceBetween:30}},on:{}});if(document.querySelector(".main-salle__slider"))new swiper_core_Swiper(".main-salle__slider",{modules:[Navigation],observer:!0,observeParents:!0,slidesPerView:2,spaceBetween:20,autoHeight:!0,speed:2e3,parallax:!0,breakpoints:{320:{slidesPerView:1.2,spaceBetween:20},640:{slidesPerView:1.2,spaceBetween:20},768:{slidesPerView:1.3,spaceBetween:20},992:{slidesPerView:1.3,spaceBetween:20},1268:{slidesPerView:2,spaceBetween:30}},on:{}});if(document.querySelector(".main-brand__slider"))new swiper_core_Swiper(".main-brand__slider",{modules:[Navigation,Autoplay],observer:!0,observeParents:!0,slidesPerView:6,spaceBetween:20,speed:2e3,autoplay:{delay:1e3},loop:!0,parallax:!0,breakpoints:{320:{slidesPerView:2,spaceBetween:20},640:{slidesPerView:4,spaceBetween:20},768:{slidesPerView:4,spaceBetween:20},992:{slidesPerView:5,spaceBetween:20},1268:{slidesPerView:6,spaceBetween:30}},on:{}});if(document.querySelector(".main-bestseller-block__slider"))new swiper_core_Swiper(".main-bestseller-block__slider",{modules:[Navigation],observer:!0,observeParents:!0,slidesPerView:1.2,spaceBetween:0,speed:2e3,parallax:!0,pagination:{el:".swiper-pagination",clickable:!0},navigation:{prevEl:".swiper-button-prev",nextEl:".swiper-button-next"},breakpoints:{445:{slidesPerView:1.7,spaceBetween:20},640:{slidesPerView:2.5,spaceBetween:0,autoHeight:!0},768:{slidesPerView:2.5,spaceBetween:20},992:{slidesPerView:3.5,spaceBetween:20},1268:{slidesPerView:5,spaceBetween:20}},on:{}});if(document.querySelector(".main-review__slider"))new swiper_core_Swiper(".main-review__slider",{modules:[Navigation],observer:!0,observeParents:!0,slidesPerView:1.2,spaceBetween:0,autoHeight:!0,speed:1e3,loop:!0,parallax:!0,pagination:{el:".swiper-pagination",clickable:!0},navigation:{prevEl:"._rebiew-prev",nextEl:"._rebiew-next"},breakpoints:{445:{slidesPerView:1.7,spaceBetween:20},640:{slidesPerView:2.5,spaceBetween:20},768:{slidesPerView:2.5,spaceBetween:20},992:{slidesPerView:3.5,spaceBetween:20},1268:{slidesPerView:3,spaceBetween:10}},on:{}})}
window.addEventListener("load",(function(e){initSliders()}));let addWindowScrollEvent=!1;function pageNavigation(){document.addEventListener("click",pageNavigationAction);document.addEventListener("watcherCallback",pageNavigationAction);function pageNavigationAction(e){if(e.type==="click"){const targetElement=e.target;if(targetElement.closest("[data-goto]")){const gotoLink=targetElement.closest("[data-goto]");const gotoLinkSelector=gotoLink.dataset.goto?gotoLink.dataset.goto:"";const noHeader=gotoLink.hasAttribute("data-goto-header")?!0:!1;const gotoSpeed=gotoLink.dataset.gotoSpeed?gotoLink.dataset.gotoSpeed:500;const offsetTop=gotoLink.dataset.gotoTop?parseInt(gotoLink.dataset.gotoTop):0;if(flsModules.fullpage){const fullpageSection=document.querySelector(`${gotoLinkSelector}`).closest("[data-fp-section]");const fullpageSectionId=fullpageSection?+fullpageSection.dataset.fpId:null;if(fullpageSectionId!==null){flsModules.fullpage.switchingSection(fullpageSectionId);document.documentElement.classList.contains("menu-open")?menuClose():null}}else gotoBlock(gotoLinkSelector,noHeader,gotoSpeed,offsetTop);e.preventDefault()}}else if(e.type==="watcherCallback"&&e.detail){const entry=e.detail.entry;const targetElement=entry.target;if(targetElement.dataset.watch==="navigator"){document.querySelector(`[data-goto]._navigator-active`);let navigatorCurrentItem;if(targetElement.id&&document.querySelector(`[data-goto="#${targetElement.id}"]`))navigatorCurrentItem=document.querySelector(`[data-goto="#${targetElement.id}"]`);else if(targetElement.classList.length)for(let index=0;index<targetElement.classList.length;index++){const element=targetElement.classList[index];if(document.querySelector(`[data-goto=".${element}"]`)){navigatorCurrentItem=document.querySelector(`[data-goto=".${element}"]`);break}}
if(entry.isIntersecting)navigatorCurrentItem?navigatorCurrentItem.classList.add("_navigator-active"):null;else navigatorCurrentItem?navigatorCurrentItem.classList.remove("_navigator-active"):null}}}
if(getHash()){let goToHash;if(document.querySelector(`#${getHash()}`))goToHash=`#${getHash()}`;else if(document.querySelector(`.${getHash()}`))goToHash=`.${getHash()}`;goToHash?gotoBlock(goToHash,!0,500,20):null}}
...
const da=new DynamicAdapt("max");da.init();const sibscribeButton=document.querySelector("._sibscribe-button");const subscribeCheckbox=document.querySelector("._subscribe-checkbox");if(sibscribeButton)sibscribeButton.disabled=!0;if(subscribeCheckbox)subscribeCheckbox.addEventListener("change",(function(){if(this.checked){if(sibscribeButton)sibscribeButton.disabled=!1}else if(sibscribeButton)sibscribeButton.disabled=!0}));const hearts=document.querySelectorAll(".heart");const likeCount=document.querySelector(".like-count");const likeCompare=document.querySelector(".compare-count");const compares=document.querySelectorAll(".compare");document.querySelectorAll(".delit");function increaseLikeCount(){likeCount.style.display="flex";likeCount.innerText=parseInt(likeCount.innerText)+1}
function decreaseLikeCount(){const currentCount=parseInt(likeCount.innerText);if(currentCount>0){likeCount.innerText=currentCount-1;if(currentCount-1===0)likeCount.style.display="none"}}
function increaseCompareCount(){likeCompare.style.display="flex";likeCompare.innerText=parseInt(likeCompare.innerText)+1}
function decreaseCompareCount(){const currentCount=parseInt(likeCompare.innerText);if(currentCount>0){likeCompare.innerText=currentCount-1;if(currentCount-1===0)likeCompare.style.display="none"}}
function deleteRowAndUpdateData(button){const index=parseInt(button.getAttribute("data-index"));const rowToDelete=button.closest("tr");rowToDelete.remove();decreaseCount(button);updateLocalStorage(index);hearts.forEach(((heart,idx)=>{if(idx===index)heart.classList.remove("heart-active");}))}
function decreaseCount(button){const countElement=button.classList.contains("delit")?likeCount:likeCompare;const currentCount=parseInt(countElement.innerText);if(currentCount>0){countElement.innerText=currentCount-1;if(currentCount-1===0)countElement.style.display="none"}}
function updateLocalStorage(index){let products=JSON.parse(localStorage.getItem("favoriteProducts"))||[];products.splice(index,1);localStorage.setItem("favoriteProducts",JSON.stringify(products))}
const deleteButtons=document.querySelectorAll(".delit");deleteButtons.forEach((button=>{button.addEventListener("click",(function(e){e.preventDefault();deleteRowAndUpdateData(button)}))}));hearts.forEach((heart=>{heart.addEventListener("click",(function(){const isActive=this.classList.contains("heart-active");if(isActive){this.classList.remove("heart-active");decreaseLikeCount();showToast('success','Успешно','Удалено из избранного')}else{this.classList.add("heart-active");increaseLikeCount();showToast('success','Успешно','Добавлено в избранное')}}))}));compares.forEach((compare=>{compare.addEventListener("click",(function(){const isActive=this.classList.contains("compare-active");if(isActive){this.classList.remove("compare-active");decreaseCompareCount()}else{this.classList.add("compare-active");increaseCompareCount()}}))}));function addRowsToTable(){const tbody=document.querySelector(".popup-favorite-table-body");tbody.innerHTML="";const activeHearts=document.querySelectorAll(".heart-active");if(activeHearts.length===0)return;const products=[];activeHearts.forEach(((heart,index)=>{const card=heart.closest(".main-bestseller-block-card");const image=card.querySelector(".main-bestseller-block-card__head-image img").getAttribute("src");const name=card.querySelector(".main-bestseller-block-card__body-title h5").textContent;const model=card.querySelector(".main-bestseller-block-card__body-type").textContent;const price=card.querySelector(".body-coast-block__price span").textContent.trim();const row=document.createElement("tr");row.innerHTML=`\n            <td class="image-table"><img src="${image}" alt="Image"></td>\n            <td class="name-table">${name}</td>\n            <td class="model-table">${model}</td>\n            <td class="availability-table">В наличии</td>\n            <td class="price-table">${price} руб</td>\n            <td class="buttons-table" >\n                <div class="buttons-table-block">\n                    <a data-popup="#popup" class="header-body__panel-cart" href="#">\n                        <span><svg>\n                            <use xlink:href="img/icons/icons.svg#cart"></use>\n                        </svg></span>\n                    </a>\n                    <span data-index="${index}" class="delit">\n                        <svg xmlns="http://www.w3.org/2000/svg" width="31" height="31" viewBox="0 0 31 31" fill="none">\n                            <path d="M8.40234 8.5L22.4023 22.5M8.40234 22.5L22.4023 8.5"  stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>\n                        </svg>\n                    </span>\n                </div>\n            </td>\n        `;tbody.appendChild(row);products.push({image,name,model,availability:"В наличии",price:price+" руб",action:"Добавить в избранное"})}));localStorage.setItem("favoriteProducts",JSON.stringify(products));const deleteButtons=document.querySelectorAll(".delit");deleteButtons.forEach((button=>{button.addEventListener("click",(function(e){e.preventDefault();deleteRowAndUpdateData(button)}))}))}
const heartElements=document.querySelectorAll(".heart");heartElements.forEach((heart=>{heart.addEventListener("click",addRowsToTable)}));document.addEventListener("DOMContentLoaded",function(){const tabSets=document.querySelectorAll("[data-tabs]");tabSets.forEach(tabSet=>{const tabButtons=tabSet.querySelectorAll(".tabs__title");const tabContents=tabSet.querySelectorAll(".tabs__body");if(tabButtons.length>0&&tabContents.length>0){tabContents[0].style.display="block";tabButtons[0].classList.add("_tab-active")}
tabSet.addEventListener("click",function(event){const clickedTab=event.target.closest(".tabs__title");if(!clickedTab)return;const selectedIndex=Array.from(tabButtons).indexOf(clickedTab);tabContents.forEach(content=>{content.style.display="none"});tabButtons.forEach(button=>{button.classList.remove("_tab-active")});tabContents[selectedIndex].style.display="block";tabButtons[selectedIndex].classList.add("_tab-active")})})});const popupSearchBoxInput=document.querySelector("._search-input");const searchDropdown=document.querySelector(".drop-search");const cartBlock=document.querySelector(".search-atem-link__cart");let isDropdownOpen=!1;if(popupSearchBoxInput&&searchDropdown){popupSearchBoxInput.addEventListener("focus",()=>{if(!popupSearchBoxInput.classList.contains("_form-focus")){popupSearchBoxInput.classList.add("_form-focus");searchDropdown.classList.add("search-dropdown-active");isDropdownOpen=!0}});popupSearchBoxInput.addEventListener("blur",()=>{setTimeout(()=>{if(!searchDropdown.contains(document.activeElement)){popupSearchBoxInput.classList.remove("_form-focus");searchDropdown.classList.remove("search-dropdown-active");isDropdownOpen=!1}},0)});document.addEventListener("click",event=>{const isCartBlockClicked=event.target===cartBlock||cartBlock?.contains(event.target);if(isDropdownOpen&&isCartBlockClicked){isDropdownOpen=!1}})}
isWebp();addTouchClass();menuInit();formFieldsInit({viewPass:!1,autoHeight:!1});formSubmit();formQuantity();pageNavigation()})()